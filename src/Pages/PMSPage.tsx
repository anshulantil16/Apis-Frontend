                  ...
                  {loginStep === 'email' ? (
                    <div className="space-y-6 animate-fadeIn">
                      <div>
                        ...
                        <input
                          ...
                          disabled={otpLoading}
                          className="w-full px-5 py-4 border-2 border-slate-200 rounded-2xl text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all font-semibold disabled:opacity-60 text-base"
                        />
                      </div>
                      <button
                        onClick={handleEmailSubmit}
                        disabled={otpLoading}
                        className="w-full px-6 py-4 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white font-black rounded-2xl shadow-lg shadow-indigo-200 transition-all active:scale-95 flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-wait text-lg"
                      >
                        {otpLoading ? <RefreshCw className="w-5 h-5 animate-spin" /> : <Zap className="w-5 h-5" />}
                    ...
                    <div className="space-y-6 animate-fadeIn">
                      ...
                      <input
                        ...
                        disabled={otpLoading}
                        maxLength={4}
                        className="w-full px-5 py-5 h-20 text-center text-6xl font-black border-3 border-indigo-300 rounded-2xl text-indigo-600 focus:outline-none focus:border-indigo-600 focus:ring-4 focus:ring-indigo-100 transition-all tracking-widest disabled:opacity-60 placeholder-slate-300"
                      />
                      <button
                        onClick={handleOtpSubmit}
                        disabled={otpLoading || otp.length !== 4}
                        className="w-full px-6 py-4 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white font-black rounded-2xl shadow-lg shadow-indigo-200 transition-all active:scale-95 flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed text-lg"
                      >
                        {otpLoading ? <RefreshCw className="w-5 h-5 animate-spin" /> : <CheckCircle className="w-5 h-5" />}
                        {otpLoading ? 'Verifying...' : 'Verify & Enter'}
                      </button>
                      <button
                        onClick={() => { setLoginStep('email'); setOtp(''); setMsg(null); setAdminEmail(''); }}
                        disabled={otpLoading}
                        className="w-full px-6 py-3 text-indigo-600 hover:text-indigo-700 font-bold text-sm transition-colors bg-indigo-50 hover:bg-indigo-100 rounded-xl disabled:opacity-60"
                      >
                        ← Try Another Email
                      </button>
