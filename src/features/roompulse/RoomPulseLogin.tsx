  ...
  return (
    <div className="relative w-full max-w-[380px] aspect-square mx-auto">
      <svg viewBox="0 0 400 400" className="w-full h-full">
        ...
        {rings.map(r => (
          <circle key={r} cx="200" cy="200" r={r} fill="none" stroke="#22d3ee" strokeOpacity="0.15" strokeWidth="1" />
        ))}
        <line x1="20" y1="200" x2="380" y2="200" stroke="#22d3ee" strokeOpacity="0.1" />
        <line x1="200" y1="20" x2="200" y2="380" stroke="#22d3ee" strokeOpacity="0.1" />
        ...
        {dots.map((dt, i) => {
          ...
          return (
            <circle key={i} cx={x} cy={y} r="4" fill="#67e8f9"
...
function Particles() {
  const pts = useRef(
    Array.from({ length: 24 }, () => ({
      ...
      d: Math.random() * 16, dur: 16 + Math.random() * 18, o: 0.15 + Math.random() * 0.35,
    }))
  ).current;
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden>
      {pts.map((p, i) => (
        <span key={i} className="absolute rounded-full bg-cyan-300"
...
export function RoomPulseLogin({ onSuccess, onBack }: {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center p-6 bg-[#050810]">
      <style>{RP_STYLES}</style>

      <div aria-hidden className="pointer-events-none absolute inset-0">
        <div className="rp-blob absolute -top-40 -left-32 w-[36rem] h-[36rem] rounded-full bg-cyan-600/20 blur-[130px]" />
        <div className="rp-blob absolute -bottom-40 -right-24 w-[36rem] h-[36rem] rounded-full bg-violet-600/20 blur-[130px]" style={{ animationDelay: '6s' }} />
        <div className="absolute inset-0 opacity-[0.35]"
          style={{ backgroundImage: 'linear-gradient(rgba(34,211,238,.05) 1px,transparent 1px),linear-gradient(90deg,rgba(34,211,238,.05) 1px,transparent 1px)',
      ...
      {onBack && (
        <button onClick={onBack}
          className="group absolute top-6 left-6 z-20 flex items-center gap-2 pl-3 pr-4 py-2.5
                     rounded-full bg-white/[0.04] backdrop-blur-xl border border-white/10
                     transition-all duration-300 hover:bg-white/[0.08] hover:-translate-x-0.5">
          ...
          <span className="text-[12px] font-black text-white/70 group-hover:text-white">Back to Hub</span>
        </button>
      )}

      <div className="relative z-10 w-full max-w-5xl grid md:grid-cols-2 rounded-[28px] overflow-hidden
                      border border-white/10 bg-white/[0.03] backdrop-blur-2xl rp-reveal
                      shadow-[0_30px_90px_-24px_rgba(6,182,212,.35)]">

        <div className="relative p-10 flex flex-col justify-between overflow-hidden
                        bg-gradient-to-br from-cyan-500/[0.06] via-transparent to-violet-500/[0.08]
          ...
          <div className="relative">
            <div className="flex items-center gap-3 mb-7">
              <div className="relative w-12 h-12 rounded-2xl bg-gradient-to-br from-cyan-400 to-violet-600
                ...
                <span className="rp-pulse-glow absolute -inset-2 rounded-2xl border-2 border-cyan-400/40" />
              </div>
              <div>
                <p className="text-white font-black text-xl tracking-tight leading-none">RoomPulse</p>
                <p className="text-[9px] font-black uppercase tracking-[0.28em] text-cyan-300/70 mt-1">
                  APIS India Limited
                </p>
              </div>
            </div>
        ...
        <div className="relative p-10 flex flex-col justify-center">
          <div className="absolute top-6 right-6 flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 rp-pulse-glow" />
          ...
          {step === 'email' ? (
            <form onSubmit={sendOtp} className="rp-reveal">
              <div className="w-11 h-11 rounded-xl bg-cyan-500/10 border border-cyan-500/25
              ...
              <label className="block text-[10px] font-black uppercase tracking-widest text-cyan-300/70 mb-2">
                Email address
              </label>
              <input type="email" value={email} autoFocus autoComplete="email"
                ...
                className="w-full px-4 py-3.5 rounded-xl bg-white/[0.04] border border-white/10
                           text-white placeholder:text-white/20 text-sm font-semibold transition-all
                           focus:outline-none focus:border-cyan-400/60 focus:bg-white/[0.07]
                           focus:ring-4 focus:ring-cyan-500/10" />
              {err && (
                <div className="flex items-start gap-2 mt-4 rounded-xl bg-rose-500/10 border border-rose-500/25 p-3 rp-reveal">
              ...
              <button type="submit" disabled={busy}
                className="relative overflow-hidden w-full mt-6 flex items-center justify-center gap-2
                           px-6 py-3.5 rounded-xl bg-gradient-to-r from-cyan-400 to-violet-600 text-white
            ...
            <div className="rp-reveal">
              ...
              <p className="text-[13px] text-white/40 mb-7 leading-relaxed">
                Sent to <b className="text-cyan-300">{masked}</b>. Expires in{' '}
                <b className="text-cyan-300 tabular-nums">{mmss}</b>.
              </p>
                ...
                {otp.map((d, i) => (
                  <input key={i} ref={el => { boxes.current[i] = el; }}
                    ...
                    className={`w-full aspect-square rounded-xl bg-white/[0.04] border text-center
                                text-2xl font-black text-white transition-all focus:outline-none
                                focus:ring-4 focus:ring-cyan-500/15
                                ${d ? 'border-cyan-400/70 bg-cyan-500/10' : 'border-white/10'} focus:border-cyan-400`} />
                ))}
              </div>
              {err && (
                <div className="flex items-start gap-2 mt-4 rounded-xl bg-rose-500/10 border border-rose-500/25 p-3 rp-reveal">
              ...
              {busy && (
                <div className="flex items-center justify-center gap-2 mt-5 text-cyan-300">
              ...
              <div className="flex items-center justify-between mt-6">
                <button onClick={() => { setStep('email'); setErr(''); }}
                  className="text-[12px] font-bold text-white/40 hover:text-cyan-300 transition-colors">
                  ← Change email
                </button>
                <button onClick={() => sendOtp()} disabled={busy || left > 240}
                  className="flex items-center gap-1.5 text-[12px] font-bold text-cyan-300
                             hover:text-cyan-200 disabled:text-white/20 transition-colors">
      ...
      <p className="absolute bottom-5 text-[10px] font-black uppercase tracking-[0.2em] text-white/15 z-10">
