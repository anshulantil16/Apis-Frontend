...
function RoomCard({ room, delay, onBook }: { room: any; delay: number; onBook: () => void }) {
  const glow = room.status === 'occupied' ? 'shadow-rose-500/20'
             : room.status === 'upcoming' ? 'shadow-amber-500/20' : 'shadow-emerald-500/10';
  const ring = room.status === 'occupied' ? 'hover:border-rose-400/40'
             : room.status === 'upcoming' ? 'hover:border-amber-400/40' : 'hover:border-emerald-400/40';
  return (
    <Reveal delay={delay}>
      <div className={`group relative rounded-2xl bg-white/[0.04] backdrop-blur-xl border
                       border-white/[0.08] p-5 shadow-xl ${glow} transition-all duration-300
                       hover:-translate-y-1.5 hover:bg-white/[0.06] ${ring}`}>
        <div className="absolute inset-x-0 top-0 h-1 rounded-t-2xl" style={{ background: room.color }} />
        <div className="flex items-start justify-between mb-3">
          <div className="min-w-0">
            <p className="text-[10px] font-black uppercase tracking-widest text-white/35 mb-0.5">
              {room.floor}
            </p>
            <h3 className="text-base font-black text-white truncate">
              {room.label && <span className="text-white/40">{room.label} </span>}{room.name}
            </h3>
          </div>
        ...
        {room.status === 'free' && !room.next_booking && (
          <p className="text-[11px] text-emerald-300/50 mb-3">Nothing booked today</p>
        )}

        <button onClick={onBook}
          className="w-full flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl
                     bg-white/[0.05] border border-white/10 text-[12px] font-black text-white/70
...
function BookingModal({ room, rooms, session, onClose, onDone }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm rp-reveal">
      <div className="w-full max-w-lg rounded-3xl bg-[#0a0f1c] border border-white/10 shadow-2xl
                      shadow-cyan-500/10 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-white/10 sticky top-0 bg-[#0a0f1c] z-10">
        ...
        {result ? (
          <div className="p-6 text-center">
            <div className={`w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center
              ${result.status === 'approved' ? 'bg-emerald-500/10' : 'bg-amber-500/10'}`}>
              <CheckCircle2 className={`w-8 h-8 ${result.status === 'approved' ? 'text-emerald-400' : 'text-amber-400'}`} />
            </div>
            <p className="text-white font-black text-lg mb-1">{result.message}</p>
          ...
          <form onSubmit={submit} className="p-5 space-y-4">
            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-cyan-300/70 mb-1.5">Room</label>
              <select value={roomId} onChange={e => setRoomId(Number(e.target.value))}
                ...
                {rooms.map(r => <option key={r.id} value={r.id} className="bg-[#0a0f1c]">{r.label} {r.name} · {r.capacity} seats</option>)}
              </select>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-cyan-300/70 mb-1.5">Date</label>
                <input type="date" value={date} min={new Date().toISOString().slice(0, 10)}
                  onChange={e => setDate(e.target.value)}
                  className="w-full px-2.5 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-white text-xs" />
              </div>
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-cyan-300/70 mb-1.5">Start</label>
                <input type="time" value={start} onChange={e => setStart(e.target.value)}
                  className="w-full px-2.5 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-white text-xs" />
              </div>
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-cyan-300/70 mb-1.5">End</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-cyan-300/70 mb-1.5">Your name</label>
                <input value={name} onChange={e => setName(e.target.value)}
                  className="w-full px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-white text-sm" />
              </div>
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-cyan-300/70 mb-1.5">Department</label>
                <input value={department} onChange={e => setDepartment(e.target.value)} placeholder="Sales"
                  className="w-full px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-white text-sm" />
              </div>
            </div>
            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-cyan-300/70 mb-1.5">Purpose</label>
              <select value={purpose} onChange={e => setPurpose(e.target.value)}
                className="w-full px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-white text-sm">
                {Object.entries(PURPOSE_LABEL).map(([k, v]) => <option key={k} value={k} className="bg-[#0a0f1c]">{v}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-cyan-300/70 mb-1.5">Details (optional)</label>
              <input value={detail} onChange={e => setDetail(e.target.value)} placeholder="What's this meeting about?"
                className="w-full px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-white text-sm" />
            </div>
            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-cyan-300/70 mb-1.5">
                Attendees
              </label>
            ...
            {err && (
              <div className="rounded-xl bg-rose-500/10 border border-rose-500/25 p-3">
                ...
                {conflict && (
                  <p className="text-[11px] text-rose-300/70 mt-2 pl-6">
...
function MyBookingsPanel({ session, refreshKey }: { session: Session; refreshKey: number }) {
  ...
  const STATUS_BADGE: Record<string, string> = {
    ...
    cancelled: 'bg-white/5 text-white/40 ring-white/10',
  };
    ...
    <div className="space-y-2.5">
      {rows.map((b, i) => (
        <Reveal key={b.id} delay={i * 40}>
          <div className="flex items-center gap-3 rounded-xl bg-white/[0.03] border border-white/[0.07] p-3.5">
            <div className="w-2 h-full min-h-[40px] rounded-full flex-shrink-0"
              style={{ background: PURPOSE_COLOUR[b.purpose] }} />
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <p className="text-[13px] font-bold text-white truncate">{b.room_name}</p>
              ...
              {b.admin_remarks && (
                <p className="text-[11px] text-white/30 mt-0.5">Note: {b.admin_remarks}</p>
              )}
            </div>
            {(b.status === 'pending' || b.status === 'approved') && (
              <button onClick={() => cancel(b.id)}
                className="px-3 py-1.5 rounded-lg text-[11px] font-bold text-rose-300/70
                           hover:text-rose-300 hover:bg-rose-500/10 transition-all flex-shrink-0">
                Cancel
              </button>
            )}
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#050810] relative">
      ...
      <div aria-hidden className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className="rp-blob absolute -top-40 -left-32 w-[32rem] h-[32rem] rounded-full bg-cyan-600/10 blur-[130px]" />
        <div className="rp-blob absolute top-1/2 -right-32 w-[32rem] h-[32rem] rounded-full bg-violet-600/10 blur-[130px]" style={{ animationDelay: '5s' }} />
      </div>

      {/* header */}
      <div className="relative z-20 bg-white/[0.02] backdrop-blur-xl border-b border-white/[0.06] sticky top-0">
        <div className="max-w-[1500px] mx-auto px-6 py-3 flex items-center gap-4">
          {onNavigateBack && (
            <button onClick={onNavigateBack}
              className="px-3 py-1.5 rounded-lg text-[12px] font-bold text-white/40 hover:text-white hover:bg-white/10 transition-all">
              ← Back
            </button>
          )}
          <div className="flex items-center gap-2.5">
            <div className="relative w-9 h-9 rounded-xl bg-gradient-to-br from-cyan-400 to-violet-600
              ...
              <span className="rp-pulse-glow absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-emerald-400 ring-2 ring-[#050810]" />
            </div>
            <div>
              <h1 className="text-lg font-black text-white tracking-tight leading-none">RoomPulse</h1>
              <p className="text-[9px] font-black uppercase tracking-[0.18em] text-white/35 mt-0.5">
          ...
          <div className="ml-auto flex items-center gap-3">
            <div className="hidden md:flex items-center gap-4 pr-4 border-r border-white/10">
              <div className="text-center">
                <p className="text-sm font-black text-emerald-300 tabular-nums">{stats.free}</p>
                <p className="text-[9px] font-bold uppercase tracking-wide text-white/30">Free</p>
              </div>
              <div className="text-center">
                <p className="text-sm font-black text-rose-300 tabular-nums">{stats.occ}</p>
                <p className="text-[9px] font-bold uppercase tracking-wide text-white/30">Occupied</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 pl-1">
              <div className="hidden sm:block text-right leading-none">
                <p className="text-[11px] font-black text-white/80">{session.name || session.email.split('@')[0]}</p>
                <p className="text-[9px] font-bold uppercase tracking-widest text-cyan-300/70">{ROLE_LABEL[session.role]}</p>
              </div>
            ...
            return (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`relative flex items-center gap-2 px-4 py-2.5 text-[13px] font-bold whitespace-nowrap
                  ${on ? 'text-cyan-300' : 'text-white/40 hover:text-white/70'}`}>
